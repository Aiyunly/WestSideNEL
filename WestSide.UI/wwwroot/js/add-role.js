function showAddRoleDialog(serverId, onSuccess, action, accountId) {
    ...
    overlay.innerHTML = `
        <div class="dialog-box" style="width:360px">
            ...
            <div class="dialog-body">
                ...
                <button class="btn-random-name" id="role-random-btn">随机英文名字</button>
                <button class="btn-random-name" id="role-random-cn-btn">随机中文名字</button>
    ...
    overlay.querySelector('#role-random-cn-btn').addEventListener('click', () => {
        overlay.querySelector('#role-name-input').value = generateRandomCnName();
    });
...
function generateRandomCnName() {
    // 随机决定名字长度：5 或 6 个字
    let length = Math.random() > 0.5 ? 6 : 5;
    let name = '';

    // 利用浏览器自带的 TextDecoder 来解码字节
    let decoder = new TextDecoder('gb2312');

    for (let i = 0; i < length; i++) {
        // GB2312 一级汉字：区码从 16 到 55
        let qu = Math.floor(Math.random() * (55 - 16 + 1)) + 16;
        
        // 位码从 1 到 94
        let wei = Math.floor(Math.random() * 94) + 1;

        // 剔除第55区最后几个没有字的空位（55区只到89位）
        if (qu === 55 && wei > 89) {
            wei = Math.floor(Math.random() * 89) + 1;
        }

        // 转化为机器字节：区码和位码分别加上 160 (0xA0)
        let byte1 = qu + 160;
        let byte2 = wei + 160;

        // 生成字节序列并解码为汉字
        let buffer = new Uint8Array([byte1, byte2]);
        name += decoder.decode(buffer);
    }
