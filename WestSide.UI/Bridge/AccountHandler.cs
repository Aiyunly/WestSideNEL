...
using WestSide.Handlers.PC.Account;
using Serilog;
...
public static class AccountHandler
{
    public static async Task<BridgeResponse> GetAccounts(BridgeRequest req)
    {
        try
        {
            await WestSide.Backend.WaitForInitAsync();
    ...
    public static async Task<BridgeResponse> RandomLogin(BridgeRequest req)
    {
        try
        {
            var (success, message) = await WestSide.Backend.RandomLogin4399Async();
            return success
        ...
        try
        {
            ...
            switch (method)
            {
                case "4399":
                {
                    ...
                    result = await WestSide.Backend.Login4399Async(account, password);
                    break;
                }
                case "netease":
                {
                    ...
                    result = await WestSide.Backend.LoginNeteaseAsync(email, password);
                    break;
                }
                case "cookie":
                {
                    ...
                    result = await WestSide.Backend.LoginCookieAsync(cookie);
                    break;
                }
                default:
