...
public static class MessageRouter
{
    ...
    public static void HandleMessage(WebView2 webView, string message)
    {
        _ = HandleMessageAsync(webView, message);
    }

    private static async Task HandleMessageAsync(WebView2? webView, string rawMessage)
    {
        ...
        try
        {
            ...
            webView.Dispatcher.Invoke(() => webView.CoreWebView2.PostWebMessageAsString(json));
        }
        catch (Exception ex)
        {
            ...
            try
            {
                ...
                {
                    Action = "error", RequestId = "", Success = false,
                    Data = JsonSerializer.SerializeToElement(new { message = ex.Message }, JsonOptions)
                };
                var errJson = JsonSerializer.Serialize(error, JsonOptions);
                webView.Dispatcher.Invoke(() => webView.CoreWebView2.PostWebMessageAsString(errJson));
            }
            catch {  }
        }
    }
