...
using WestSide.Handlers.PC;
using WestSide.Handlers.Game.NetServer;
    ...
    public static async Task<BridgeResponse> ListSessions(BridgeRequest req)
    {
        try
        {
            ...
            var items = result.GetType().GetProperty("items")?.GetValue(result)
                as IEnumerable<WestSide.Entities.Web.NEL.EntityQueryGameSessions>;

            var sessions = (items ?? Enumerable.Empty<WestSide.Entities.Web.NEL.EntityQueryGameSessions>())
                .Select(s => new
                {
                    id = s.Id,
