...
using WestSide.Entities.Web.RentalGame;
using WestSide.Handlers.PC.Game.RentalServer;
using WestSide.Handlers.PC.Account;
using WestSide.Handlers.PC.Game.NetGame;
using WestSide.Manager;
using Serilog;
using GetRoleNamed = WestSide.Handlers.PC.Game.RentalServer.GetRoleNamed;
...
public static class RentalHandler
{
    ...
    public static async Task<BridgeResponse> JoinServer(BridgeRequest req)
    {
        ...
        try
        {
            ...
            var result = await Task.Run(() => new JoinRentalGame().Execute(joinRequest));
